...
package net.nashlegend.sourcewall.view.common.ScaledImage;
...
import net.nashlegend.sourcewall.R.styleable;
import net.nashlegend.sourcewall.view.common.ScaledImage.decoder.CompatDecoderFactory;
import net.nashlegend.sourcewall.view.common.ScaledImage.decoder.DecoderFactory;
import net.nashlegend.sourcewall.view.common.ScaledImage.decoder.ImageDecoder;
import net.nashlegend.sourcewall.view.common.ScaledImage.decoder.ImageRegionDecoder;
import net.nashlegend.sourcewall.view.common.ScaledImage.decoder.SkiaImageDecoder;
import net.nashlegend.sourcewall.view.common.ScaledImage.decoder.SkiaImageRegionDecoder;
...
/**
 * Copied from https://github.com/davemorrissey/subsampling-scale-image-view;
 * <p>
...
public class SubsamplingScaleImageView extends View {
    ...
    public SubsamplingScaleImageView(Context context, AttributeSet attr) {
        ...
        if (attr != null) {
            ...
            if (typedAttr.hasValue(styleable.SubsamplingScaleImageView_assetName)) {
                String assetName = typedAttr.getString(
                        styleable.SubsamplingScaleImageView_assetName);
     ...
     * on the imageSource object. The preview source will be ignored if you don't provide
     * dimensions,
     ...
     * Set the image source from a bitmap, resource, asset, file or other URI, providing a preview
     * image to be
     * displayed until the full size image is loaded, starting with a given orientation setting,
     * scale and center.
    ...
    public final void setImage(ImageSource imageSource, ImageSource previewSource,
        ...
        if (previewSource != null) {
            ...
            if (previewSource.getBitmap() != null) {
                ...
                BitmapLoadTask task = new BitmapLoadTask(this, getContext(), bitmapDecoderFactory,
                        uri, true);
                execute(task);
            }
        }
        ...
        if (imageSource.getBitmap() != null && imageSource.getSRegion() != null) {
            ...
            if (imageSource.getTile() || sRegion != null) {
                ...
                TilesInitTask task = new TilesInitTask(this, getContext(), regionDecoderFactory,
                        uri);
                execute(task);
            } else {
        ...
        this.detector = new GestureDetector(context, new GestureDetector.SimpleOnGestureListener() {
            ...
            public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
                ...
                if (panEnabled && readySent && vTranslate != null && e1 != null && e2 != null && (
                        Math.abs(e1.getX() - e2.getX()) > 50 || Math.abs(e1.getY() - e2.getY())
                                > 50) && (Math.abs(velocityX) > 500 || Math.abs(velocityY) > 500)
                        && !isZooming) {
            ...
            public boolean onDoubleTap(MotionEvent e) {
                if (zoomEnabled && readySent && vTranslate != null) {
                    // Hacky solution for #15 - after a double tap the GestureDetector gets in a
                    // state
    ...
    public boolean onTouchEvent(@NonNull MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (maxTouchCount > 0) {
                    if (touchCount >= 2) {
                        // Calculate new distance between touch points, to scale and pan relative
                        // to start values.
                        ...
                        if (zoomEnabled && (distance(vCenterStart.x, vCenterEndX, vCenterStart.y,
                                vCenterEndY) > 5 || Math.abs(vDistEnd - vDistStart) > 5
                                || isPanning)) {
                            isZooming = true;
                            isPanning = true;
                            consumed = true;
                        ...
                        float dist = Math.abs(vCenterStart.y - event.getY()) * 2
                                + quickScaleThreshold;
    ...
    private void doubleTapZoom(PointF sCenter, PointF vFocus) {
        ...
        if (doubleTapZoomStyle == ZOOM_FOCUS_CENTER_IMMEDIATE) {
            ...
            new AnimationBuilder(targetScale, sCenter).withInterruptible(false).withDuration(
                    doubleTapZoomDuration).start();
        } else if (doubleTapZoomStyle == ZOOM_FOCUS_FIXED) {
            new AnimationBuilder(targetScale, sCenter, vFocus).withInterruptible(
                    false).withDuration(doubleTapZoomDuration).start();
        }
        invalidate();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // If image has been loaded or supplied as a bitmap, onDraw may be the first time the
        // view has
        // dimensions and therefore the first opportunity to set scale and translate. If this
        // call returns
        ...
        if (anim != null) {
            ...
            // Find out where the focal point is at this scale and adjust its position to follow
            // the animation path
        ...
        if (tileMap != null && isBaseLayerReady()) {
            ...
            // First check for missing tiles - if there are any we need the base layer underneath
            // to avoid gaps
            ...
            // Render all loaded tiles. LinkedHashMap used for bottom up rendering - lower res
            // tiles underneath.
            for (Map.Entry<Integer, List<Tile>> tileMapEntry : tileMap.entrySet()) {
                        ...
                        if (tile.visible && debug) {
                            canvas.drawText(
                                    "ISS " + tile.sampleSize + " RECT " + tile.sRect.top + ","
                                            ...
                                            + tile.sRect.right, tile.vRect.left + 5,
            ...
            if (debug) {
                canvas.drawText("Scale: " + String.format(Locale.ENGLISH, "%.2f", scale), 5, 15,
                        debugPaint);
                canvas.drawText(
                        "Translate: " + String.format(Locale.ENGLISH, "%.2f", vTranslate.x) + ":"
                                + String.format(Locale.ENGLISH, "%.2f", vTranslate.y), 5, 35,
                        debugPaint);
                PointF center = getCenter();
                canvas.drawText(
                        "Source center: " + String.format(Locale.ENGLISH, "%.2f", center.x) + ":"
                                + String.format(Locale.ENGLISH, "%.2f", center.y), 5, 55,
                        debugPaint);
    ...
    private synchronized void initialiseBaseLayer(Point maxTileDimensions) {
        ...
        // Load double resolution - next level will be split into four tiles and at the center
...
//        }
        ...
        if (fullImageSampleSize == 1 && sRegion == null && sWidth() < maxTileDimensions.x
            ...
            decoder.recycle();
            decoder = null;
            BitmapLoadTask task = new BitmapLoadTask(this, getContext(), bitmapDecoderFactory, uri,
                    false);
            execute(task);
    ...
    private void refreshRequiredTiles(boolean load) {
        ...
        // and those that are higher
        // resolution than required, or lower res than required but not the base layer, so the
     ...
     *               While animating this can be false to avoid changes in direction as bounds are
     *               reached.
     * @param sat    The scale we want and the translation we're aiming for. The values are adjusted
     *               to be valid.
     */
    private void fitToBounds(boolean center, ScaleAndTranslate sat) {
        ...
        float yPaddingRatio =
                getPaddingTop() > 0 || getPaddingBottom() > 0 ? getPaddingTop() / (float) (
                        getPaddingTop() + getPaddingBottom()) : 0.5f;
    ...
    private static class BitmapLoadTask extends AsyncTask<Void, Void, Integer> {
        ...
        public BitmapLoadTask(SubsamplingScaleImageView view, Context context,
            ...
            this.viewRef = new WeakReference<SubsamplingScaleImageView>(view);
            this.contextRef = new WeakReference<Context>(context);
            this.decoderFactoryRef = new WeakReference<DecoderFactory<? extends ImageDecoder>>(
                    decoderFactory);
            this.source = source;
    ...
    private int getExifOrientation(String sourceUri) {
        ...
        if (sourceUri.startsWith(ContentResolver.SCHEME_CONTENT)) {
            ...
            try {
                ...
                cursor = getContext().getContentResolver().query(Uri.parse(sourceUri), columns,
                        null, null, null);
                if (cursor != null) {
    ...
    private void execute(AsyncTask<Void, Void, ?> asyncTask) {
        if (parallelLoadingEnabled && VERSION.SDK_INT >= 11) {
            try {
                ...
                Method executeMethod = AsyncTask.class.getMethod("executeOnExecutor",
                        Executor.class, Object[].class);
                executeMethod.invoke(asyncTask, executor, null);
                return;
            } catch (Exception e) {
                Log.i(TAG,
                        "Failed to execute AsyncTask on thread pool executor, falling back to "
     ...
     * Tile#TILE_SIZE_AUTO} will re-enable the default behaviour.
     */
    ...
    private Point getMaxBitmapDimensions(Canvas canvas) {
        ...
        if (VERSION.SDK_INT >= 14) {
            try {
                ...
                maxHeight = (Integer) Canvas.class.getMethod("getMaximumBitmapHeight").invoke(
                        canvas);
            } catch (Exception e) {
                // Return default
            }
     ...
     * significant memory.
     * After you have called this method, the view can be re-used by setting a new image. Settings
     * are remembered
    ...
    public final void setRegionDecoderClass(
        ...
        this.regionDecoderFactory = new CompatDecoderFactory<ImageRegionDecoder>(
                regionDecoderClass);
    }
     ...
     * to set this according
     * to screen density - on a retina screen, 1:1 may still be too small. Consider using {@link
     * #setMinimumDpi(int)},
     ...
     * express the maximum
     * allowed scale in terms of the minimum pixel density. This avoids the problem of 1:1 scale
     * still being
     * too small on a high density screen. A sensible starting point is 160 - the default used by
     * this view.
     *
     ...
     * and the full size image may still be loading. If no preview was provided, this is called
     * once
     ...
     * as a zoom out gesture - if the scale is greater than 90% of this value, a double tap zooms
     * out. Avoid using values
     ...
     * image will zoom in to when double tapped in terms of the image pixel density. Values lower
     * than the max scale will
     ...
     * there
     * is a possibility that it will make the tile loading unreliable, but it reduces the chances
     * of
     ...
     * {@link SubsamplingScaleImageView.AnimationBuilder#start()}
     * to start the anim.
     */
    ...
    public final class AnimationBuilder {
         ...
         * point - the nearest
         * point to the center allowed by pan limits. When false, animation is in the direction of
         * the requested end
         * point and is stopped when the limit for each axis is reached. The latter behaviour is
         * used for flings but
         * nothing else.
         */
        ...
        public void start() {
            ...
            PointF targetSCenter = panLimited ? limitedSCenter(this.targetSCenter.x,
                    this.targetSCenter.y, targetScale, new PointF()) : this.targetSCenter;
            anim = new Anim();
     ...
     * these events are triggered if the activity is paused, the image is swapped, or in other
     * cases
    ...
    public interface OnImageEventListener {
         ...
         * translate
         * are known and the next draw will display an image. This event can be used to hide a
         * loading
         ...
         * This event could be used as a trigger to enable gestures if you wanted interaction
         * disabled
         * while only a preview is displayed, otherwise for most cases {@link #onReady()} is the
         * best
         * event to listen to.
         */
        void onImageLoaded();
         ...
         * loaded
         * and displayed with no detectable error. The view will continue to load the full size
         * image.
         *
        ...
        /**
         * Indicates an error initiliasing the decoder when using a tiling, or when loading the
         * full
        ...
        /**
         * Called when an image tile could not be loaded. This method cannot be relied upon;
         * certain
         * encoding types of supported image formats can result in corrupt or blank images being
         * loaded
         * and displayed with no detectable error. Most cases where an unsupported file is used
         * will
